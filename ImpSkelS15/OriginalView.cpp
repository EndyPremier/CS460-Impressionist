...
OriginalView* OriginalView::instance = nullptr;
...
OriginalView* OriginalView::getInstance()
{
  return instance;
}
...
void OriginalView::draw()
{
	...
	if ( *currentView )
	{
		...
		bitstart = *currentView + 3 * ((m_pDoc->m_nWidth * startrow) + scrollpos.x);
...
void OriginalView::setView(int pic)
{
  if (m_pDoc != NULL)
  {
    switch (pic)
    {
      case 0:  currentView = &(m_pDoc->m_ucBitmap);        break;
      case 1:  currentView = &(m_pDoc->m_ucGrayscaleMap);  break;
      case 2:  currentView = &(m_pDoc->m_ucHorEdgeMap);    break;
      case 3:  currentView = &(m_pDoc->m_ucVerEdgeMap);    break;
    }
  }
  redraw();
}
